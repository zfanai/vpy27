...
class IOLoop(Configurable):
    ...
    def run_sync(self, func, timeout=None):
        ...
        def run():
            try:
                print ('run_sync:run:1')
                result = func()
                print('run_sync:run:2', result)
                if result is not None:
                    print('run_sync:run:3', result)
                    from tornado.gen import convert_yielded
                    result = convert_yielded(result)
                    print('run_sync:run:3.1:', result)
            except Exception:
                print('run_sync:run:4')
                future_cell[0] = TracebackFuture()
                future_cell[0].set_exc_info(sys.exc_info())
            else:
                print('run_sync:run:5')
                if is_future(result):
                    future_cell[0] = result
                else:
                    future_cell[0] = TracebackFuture()
                    future_cell[0].set_result(result)
            print('run_sync:run:6:', future_cell)
            # closure 
    ...
    def start(self):
        ...
        try:
            while True:
                ...
                if self._timeouts:
                    now = self.time()
                    print ('PIOL:s:', self._timeouts) 
                    while self._timeouts:
                        print ('PIOL:s5:', self._timeouts[0].callback, self._timeouts[0].deadline) 
                ...
                try:
                    print ('select poll:5,', poll_timeout)
                ...
                print ('select poll:6,', event_pairs)
