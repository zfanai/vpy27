...
def constant_time_compare(val1, val2):
    ...
    """
    debug.trace('itsdangerous', 'constant_time_compare,1')
    #print val1
    #print val2
...
class SigningAlgorithm(object):
    ...
    def verify_signature(self, key, value, sig):
        ...
        debug.trace('itsdangerous', 'verify_signature,1')
...
class Signer(object):
    ...
    def verify_signature(self, value, sig):
        ...
        trv=self.algorithm.verify_signature(key, value, sig)
        debug.trace('itsdangerous', 'verify_signature,2', trv)
        return trv

    def unsign(self, signed_value):
        """Unsigns the given string."""
        debug.trace('itsdangerous', 'unsign,366', signed_value)
        signed_value = want_bytes(signed_value)
        debug.trace('itsdangerous', 'unsign,2', signed_value)
        sep = want_bytes(self.sep)
        debug.trace('itsdangerous', 'unsign,3', sep, self.sep)
        if sep not in signed_value:
            raise BadSignature('No %r found in value' % self.sep)
        value, sig = signed_value.rsplit(sep, 1)
        debug.trace('itsdangerous', 'unsign,4', value, sig)
        #debug.print_stack('itsdangerous', 'unsign,5')
        if self.verify_signature(value, sig):
            debug.trace('itsdangerous', 'unsign,6', value)
            return value
...
class TimestampSigner(Signer):
    ...
    def unsign(self, value, max_age=None, return_timestamp=False):
        ...
        if max_age is not None:
            ...
            debug.trace('itsdangerous', 'TimestampSigner,unsign,1', age, max_age)
            if age > max_age:
...
class TimedSerializer(Serializer):
    ...
    def loads(self, s, max_age=None, return_timestamp=False, salt=None):
        ...
        """
        debug.trace('itsdangerous', 'TimedSerializer,loads,1')
        signer1=self.make_signer(salt)
        debug.trace('itsdangerous', 'TimedSerializer,loads,1.1', signer1, type(signer1))
        trv=signer1.unsign(s, max_age, return_timestamp=True)
        debug.trace('itsdangerous', 'TimedSerializer,loads,1.2', trv)
        base64d, timestamp = trv
        #base64d, timestamp = self.make_signer(salt) \
        #    .unsign(s, max_age, return_timestamp=True)
        
        debug.trace('itsdangerous', 'TimedSerializer,loads,2')
        payload = self.load_payload(base64d)
        debug.trace('itsdangerous', 'TimedSerializer,loads,3')
        if return_timestamp:
